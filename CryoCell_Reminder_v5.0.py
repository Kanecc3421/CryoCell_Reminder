...
def test_email():
    ...
    if not config:
        messagebox.showwarning("提示", "请先配置邮箱信息")
        return
...
def send_reminders():
    try:
        ...
        server = smtplib.SMTP_SSL(smtp_server, smtp_port)
        server.login(EMAIL_USER, EMAIL_PASSWORD)
        log_message("SMTP登录成功")

        success_count = 0
        fail_count = 0
        
        for rec in to_notify:
            id_, name, freeze_date, remind_date = rec
            subject = f"请将'{name}'细胞放入液氮罐"
            
            # 计算天数差异
            try:
                freeze_date_obj = datetime.datetime.strptime(freeze_date, "%Y-%m-%d").date()
                remind_date_obj = datetime.datetime.strptime(remind_date, "%Y-%m-%d").date()
                days_diff = (datetime.date.today() - remind_date_obj).days
                
                if days_diff > 0:
                    days_text = f"已经逾期 {days_diff} 天"
                elif days_diff == 0:
                    days_text = "今天是最后期限"
                else:
                    days_text = f"还有 {-days_diff} 天"
            except:
                days_text = ""
        ...
        log_message(f"邮件发送完成: 成功 {success_count} 条, 失败 {fail_count} 条")
        return True
    except Exception as e:
...
def run_scheduler():
    log_message("定时任务启动")
    # 添加详细的日志记录
    schedule.every().day.at("09:00").do(lambda: log_message("定时任务检查..."))
    
    # 主任务 - 每天09:30发送提醒
    schedule.every().day.at("09:30").do(lambda: send_reminders())
    
    # 测试任务 - 每5分钟检查一次（用于调试）
    schedule.every(5).minutes.do(lambda: log_message("定时任务运行中..."))
...
def manage_boxes():
    class BoxManagerWindow:
        ...
        def rename_box(self):
            ...
            tk.Label(frame, text="新名称:", bg="white", font=("Arial", 11)).pack(pady=5)
            ...
            tk.Button(btn_frame, text="确认", command=confirm_rename, 
                     bg="#4CAF50", fg="white", font=("Arial", 11)).pack(side=tk.LEFT, padx=10)
            tk.Button(btn_frame, text="取消", command=popup.destroy, 
...
def create_gui():
    ...
    tk.Button(toolbar, text="测试邮件", command=test_email, **btn_style).pack(side=tk.LEFT, padx=8, pady=5)   
...
if __name__ == "__main__":
    ...
    # 添加手动发送按钮
    manual_frame = tk.Frame(app, bg="white")
    manual_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=10)
    
    tk.Button(manual_frame, text="立即检查提醒", command=send_reminders, 
             bg="#4CAF50", fg="white", font=("Arial", 11, "bold")).pack()
